...
public class UniquePersonList implements Iterable<Person> {
     ...
     * {@code editedPerson} must not be the same as another existing person in
     * the list.
     */
    public void setPerson(Person target, Person editedPerson) {
        ...
        boolean editedPersonWouldDuplicateAnother = false;
        for (int i = 0; i < internalList.size(); i++) {
            if (i == index) {
                continue;
            }
            if (editedPerson.isSamePerson(internalList.get(i))) {
                editedPersonWouldDuplicateAnother = true;
                break;
            }
        }

        if (editedPersonWouldDuplicateAnother) {
            throw new DuplicatePersonException();
        }
    ...
    /**
     * Replaces the contents of this list with {@code persons}. {@code persons}
     * must not contain duplicate persons.
     */
